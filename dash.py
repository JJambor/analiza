...
def load_data():
    ...
    for file in files:
        try:
            ...
            with st.container():
                ...
                time.sleep(1)  # Skrócono czas wyświetlania
                placeholder.empty()
    ...
    return df.dropna(subset=["Data_full"])
...
# Jawne rzutowanie typów na początku
...
df_filtered = df_filtered[df_filtered["Login POS"] != "99999"]
...
def plot_line_chart(df, x, y, color, title, key_suffix="", fill_area=False):
    try:
        ...
        with col_fav:
            ...
            is_active = fav_key in st.session_state.favorite_charts
            if st.button("★", key=f"fav_btn_{fav_key}", help="Dodaj do ulubionych" if not is_active else "Usuń z ulubionych"):
                if is_active:
...
with tab1:
    ...
    else:  # Penetracja lojalnościowa
        all_tx = df_filtered.groupby(["Dzień tygodnia", "Godzina"])["#"].nunique().rename("Wszystkie")
        loyal_tx = df_filtered[df_filtered["Karta lojalnościowa"].str.upper() == "TAK"].groupby(["Dzień tygodnia", "Godzina"])["#"].nunique().rename("Lojalnościowe")
...
with tab2:
    ...
    avg_mies_df = df_filtered[df_filtered["HOIS"] != "0"].groupby("Okres")["Netto"].sum().to_frame().join(
        df_filtered.groupby("Okres")["#"].nunique()
    ).reset_index()
    ...
    if category_col == "Stacja":
        avg_mies_stacje_df = df_filtered[df_filtered["HOIS"] != "0"].groupby(["Okres", "Stacja"])["Netto"].sum().to_frame().join(
            df_filtered.groupby(["Okres", "Stacja"])["#"].nunique()
        ).reset_index()
...
with tab4:
    ...
    df_prev_filtered = df[(df["Data"] >= start_date_prev.date()) & (df["Data"] <= end_date_prev.date())].copy()
    df_prev_filtered["Grupa towarowa"] = df_prev_filtered["HOIS"].map(lambda x: hois_map.get(x, ("Nieznana", "Nieznana"))[0])
    df_prev_filtered["Grupa sklepowa"] = df_prev_filtered["HOIS"].map(lambda x: hois_map.get(x, ("Nieznana", "Nieznana"))[1])
    df_prev_filtered = df_prev_filtered[(df_prev_filtered["Stacja"].isin(selected_stations)) & (df_prev_filtered["Grupa towarowa"].isin(selected_groups)) & (df_prev_filtered["Login POS"] != "99999")]
    ...
    df_both_melted = pd.merge(loyal_daily, total_daily, on="Okres").melt(id_vars=["Okres"], value_vars=["Lojalnościowe", "Wszystkie"], var_name="Typ transakcji", value_name="Liczba")
...
with tab7:
    ...
    if not top_products_df.empty:
        ...
        df_top = df_filtered[(df_filtered["Miesiąc"] == wybrany_miesiac) & (df_filtered["PLU"].isin(top_plu_list))].copy()
        if not df_top.empty:
        ...
        loyal_tx = df_filtered[df_filtered["Karta_TAK"]].groupby("Kasjer")["#"].nunique().reset_index().rename(columns={"#": "Lojalnościowe"})
